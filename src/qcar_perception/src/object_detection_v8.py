...
class Detector:
    def __init__(self):
        ...
        self.is_yolo_v8 = "yolov8" in self.weights_path.lower()
        ...
        # Load model
        self.model = YOLO(self.weights_path)  # load FP32 model
        self.stride = int(self.model.model.model[-1].stride.max())      # model stride
    ...
    def image_cb(self, data):
        ...
        with torch.no_grad():
            detection = self.model.predict(input_img, conf=self.conf_thres, iou=self.iou_thres)[0]
            print("detections", detection)
            print("Boxes", detection.boxes)
 
        # Parse detections
        if detection.boxes.cls is not None:
            for idx in range(len(detection.boxes.cls)):
                # Get xmin, ymin, xmax, ymax, confidence and class
                xmin, ymin, xmax, ymax = detection.boxes.data[idx, :4]
                conf, det_class = detection.boxes.conf[idx], detection.boxes.cls[idx]
